...
The former #95 reconciliation is resolved by the [canonical ordering/admission/activation architecture](./architecture-canonical-ordering-admission-activation.md). No command may query or mutate half-activated destination state. Known finite unavailable dependencies are preflighted/staged to a fixed semantic frontier before first authoritative use; a dynamic finite dependency discovered after an operation has already committed a prefix causes same-point semantic suspension, activation/catch-up, atomic publication and exact-once resumption. Canonical intervals are half-open `[N,N+1)`; per-domain frontier markers prevent catch-up and active work from processing the same occurrence twice.


**SPAT95-01 — unavailable teleport:** inject destination load failure and verify source WorldPosition/SpatialCell/index remain intact. On success, destination occupancy is fully caught up and published before movement revalidation/commit; a later stale movement does not roll back already-shared published activation.
