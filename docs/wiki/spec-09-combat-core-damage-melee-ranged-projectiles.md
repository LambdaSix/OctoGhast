...
Within one deterministic server resolution boundary, an admitted immediate Cataclysm-profile attack is atomic with respect to externally observable authoritative state. The [canonical ordering/admission/activation architecture](./architecture-canonical-ordering-admission-activation.md) resolves the former #95 gate: PlayerId/network admission order only selects the bounded admitted set; combat contention follows the Cataclysm profile phase/lane and stable actor/work execution order. A later action revalidates the state left by earlier actions.
...
**COMBAT95-01 — reversed PlayerId/ActorId contention:** admit two attacks in one canonical cut where PlayerId order is opposite controlled ActorId order. With equal phase/due/work priority, ActorId/profile execution order determines the first committed attack; the later attack revalidates the resulting target/weapon state.
